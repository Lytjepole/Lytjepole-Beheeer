...
switch ($_GET['action']) {
    ...
    case 'create':
        ...
        for($i = 0; $i < count($data); $i++) {
            ...
            $i = 1;
            while (file_exists('../../images/users/' . $testName)) {
            ...
            $canvas->writeImage('../../images/users/'.$testName);
            $origcanvas->resizeImage(75, 75, imagick::FILTER_CATROM, 1, true);
            $origcanvas->writeImage('../../images/users/75x75/'.$testName);

            //update database

            $sql = "INSERT INTO `userImages` (`imageName`, `imagePath`, `artist`, `ownerId`, `recentlyUsed`) VALUES ('test', '".$testName."', 'pipo', '1', NOW());";
            $database->query($sql);
        }
...
//        break;
    ...
    default:
        if (isset($_GET['id'])) { // return image by id
            ...
            $imageId = $_GET['id'];
            $sql = "SELECT `id`, `imageName`, `imagePath`, `artist`, `ownerId`, `recentlyUsed` FROM `userImages` WHERE `id` = ?";
            ...
            if($stmt->num_rows) {
                echo '{"success": true, "total": 1, "userimage":' . json_encode($arr) . '}';
            }else {
                echo '{"success": false}';
            }
            ...
            if (mysqli_num_rows($result)) {
                ...
                echo '{"success": true, "total": ' . $total[0] . ', "userimage":' . json_encode($arr) . '}';
            } else {
